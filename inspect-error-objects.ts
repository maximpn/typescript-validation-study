...
const fleetMultiFailureData = testCases.filter(
  (testcase) =>
    testcase.schema === 'fleet' && testcase.id === 'multiple-failure'
)[0];
...
for (let [solName, solModule] of solutions) {
  ...
  if (validator !== undefined) {
    console.log(
      `********************************* Solution [${solName}] Validating multiple-failure fleet data *********************************`
    );
    try {
      ...
      console.log(
        `JSON.stringify(response)=${JSON.stringify(response, null, 2)}`
      );
    }
  }
}
