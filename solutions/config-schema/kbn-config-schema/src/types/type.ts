...
export const convertValidationFunction = <T = unknown>(
  ...
  return (value, { error }) => {
    ...
    try {
      ...
      validationResultMessage = (e as any).message || e;
    }
...
export abstract class Type<V> {
  ...
  public validate(
    ...
    const { value: validatedValue, error } = this.internalSchema.validate(
      value,
      {
        context,
        presence: 'required',
...
function recursiveGetSchemaStructure(
  internalSchema: AnySchema,
  path: string[] = []
) {
