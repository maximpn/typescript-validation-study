...
describe("Yup schema traversable test", () => {
  ...
  it("is able to find the item type of fleet schema (object)", () => {
    expect((fleetSchema.describe().innerType as any)?.type).toBe("object");
  });
  it("is further able to find all keys in the item object of fleet schema", () => {
    expect(
      Object.keys((fleetSchema.describe().innerType as any).fields).length
    ).toBe(2);
    expect(
      Object.keys((fleetSchema.describe().innerType as any).fields)
    ).toContain("driver");
    expect(
      Object.keys((fleetSchema.describe().innerType as any).fields)
    ).toContain("vehicle");
