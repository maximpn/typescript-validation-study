...
const validPersonData = testCases.filter(
  (testcase) =>
    testcase.schema === 'person' && testcase.id === 'sex-none-ok-optional'
)[0];
...
for (let [solName, solModule] of solutions) {
  ...
  if (validator !== undefined) {
    console.log(
      `********************************* Solution [${solName}] Validating valid person data *********************************`
    );
    const result = validator(_.cloneDeep(validPersonData.data));
    console.log(`result = ${result}`);
    console.log(
      `TYPE COERCION = ${
        result && result.dob && result.dob instanceof Date ? 'TRUE' : 'FALSE'
      }`
    );
    console.log(
      `DEFAULT = ${
        result && result.sex && result.sex === 'O' ? 'TRUE' : 'FALSE'
      }`
    );
